...
app = FastAPI(
    ...
    description="Cette API permet d'extraire le contenu des pages web en utilisant différents services.",
    version="1.0.0",
)
...
class ExtractionEngine(str, Enum):
    tavily = "tavily"
    exa = "exa"
    auto = "auto"  # Choisit automatiquement le moteur disponible
...
class ExtractionRequest(BaseModel):
    ...
    engine: ExtractionEngine = Field(default=ExtractionEngine.auto, description="Le moteur d'extraction à utiliser")
...
async def check_api_health(engine: ExtractionEngine) -> ApiStatus:
    ...
    try:
        if engine == ExtractionEngine.exa:
            if not EXA_KEY:
                return ApiStatus.unavailable
            url = "https://api.exa.ai/health"  # Endpoint fictif, ajustez selon l'API réelle
            headers = {"Authorization": f"Bearer {EXA_KEY}"}
            
        elif engine == ExtractionEngine.tavily:
            if not TAVILY_KEY:
                return ApiStatus.unavailable
...
async def get_best_available_engine() -> ExtractionEngine:
    """Détermine le meilleur moteur disponible basé sur les vérifications de santé"""
    engines = [ExtractionEngine.exa, ExtractionEngine.tavily]
    statuses = await asyncio.gather(*[check_api_health(engine) for engine in engines])
    
    # Priorité aux APIs qui sont complètement OK
    for engine, status in zip(engines, statuses):
        if status == ApiStatus.ok:
            return engine
    
    # Sinon, utiliser une API dégradée
    for engine, status in zip(engines, statuses):
        if status == ApiStatus.degraded:
            return engine
    
    # Si toutes sont indisponibles, lever une exception
    raise HTTPException(
        status_code=503,
        detail="Tous les services d'extraction sont actuellement indisponibles. Veuillez réessayer plus tard."
    )
...
async def extract_with_exa(urls: List[str], include_images: bool = False, include_raw_content: bool = False, include_links: bool = False) -> Dict[str, Any]:
    """Extraction de contenu avec Exa AI avec gestion des erreurs et retries"""
        ...
        try:
            async with httpx.AsyncClient(timeout=TIMEOUT_SECONDS) as client:
                ...
                if response.status_code == 200:
                    ...
                    if attempt < MAX_RETRIES:
                        wait_time = min(2 ** attempt, 10)  # Exponential backoff
                        logger.warning(f"Rate limit hit with Exa API. Retrying in {wait_time}s. Attempt {attempt+1}/{MAX_RETRIES+1}")
                        await asyncio.sleep(wait_time)
                        continue
                
                # Autres erreurs
                error_detail = f"Erreur de l'API Exa: {response.status_code}"
                try:
                    error_detail += f" - {response.json().get('error', '')}"
                except:
                    error_detail += f" - {response.text[:100]}"
            ...
            if attempt < MAX_RETRIES:
                logger.warning(f"Timeout with Exa API. Retrying. Attempt {attempt+1}/{MAX_RETRIES+1}")
                continue
            raise HTTPException(status_code=504, detail="Timeout lors de la requête à l'API Exa")
            
        except httpx.RequestError as e:
            if attempt < MAX_RETRIES:
                logger.warning(f"Request error with Exa API: {str(e)}. Retrying. Attempt {attempt+1}/{MAX_RETRIES+1}")
                continue
            raise HTTPException(status_code=502, detail=f"Erreur de requête à l'API Exa: {str(e)}")
            
        except Exception as e:
            logger.error(f"Unexpected error with Exa API: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Erreur inattendue lors de la requête à l'API Exa: {str(e)}")
    
    # Si on arrive ici, c'est qu'on a épuisé les tentatives
    raise HTTPException(status_code=429, detail="Trop de tentatives échouées avec l'API Exa")
...
def process_exa_results(data: Dict[str, Any]) -> List[UrlExtractionResult]:
    """Traite les résultats bruts d'Exa en format standardisé"""
    results = []
    
    for item in data.get("results", []):
        # Extraire les sous-pages si présentes
        subpages = []
        for subpage in item.get("subpages", []):
            subpage_data = {
                "url": subpage.get("url"),
                "title": subpage.get("title"),
                "content": subpage.get("text"),
                "author": subpage.get("author"),
                "published_date": subpage.get("publishedDate")
            }
            subpages.append(subpage_data)
        ...
        result = UrlExtractionResult(
            ...
            images=[{"url": item.get("image")}] if item.get("image") else [],
            links=item.get("extras", {}).get("links", []),
            subpages=subpages
        )
        results.append(result)
...
async def extract_content(request: Request, extraction_request: ExtractionRequest):
    ...
    - **engine**: Le moteur d'extraction à utiliser (tavily, exa, auto)
    ...
    start_time = time.time()
    engine = extraction_request.engine
    ...
    # Sélectionner automatiquement le meilleur moteur disponible si nécessaire
    if engine == ExtractionEngine.auto:
        try:
            engine = await get_best_available_engine()
            logger.info(f"[{request_id}] Auto-selected engine: {engine}")
        except Exception as e:
            logger.error(f"[{request_id}] Failed to auto-select engine: {str(e)}")
            return ExtractionResponse(
                ...
                error=Error(
                    ...
                    message="Aucun moteur d'extraction n'est actuellement disponible",
                    details={"error": str(e)}
                )
            )
    ...
    for url in urls:
        ...
        cache_key = generate_cache_key(engine, url, include_images, include_raw_content, extract_depth.value)
        ...
        try:
            ...
            if engine == ExtractionEngine.tavily:
                # Tavily permet d'extraire plusieurs URLs en une seule requête
                # mais pour simplifier la gestion du cache, on fait une requête par URL
            ...
            elif engine == ExtractionEngine.exa:
                data = await extract_with_exa([url], include_images, include_raw_content, include_links)
                url_results = process_exa_results(data)
...
async def health_check():
    ...
    # Vérifier l'état des API externes
    engines = [ExtractionEngine.exa, ExtractionEngine.tavily]
    statuses = {}
    
    try:
        # Vérifier les API en parallèle
        results = await asyncio.gather(
            *[check_api_health(engine) for engine in engines],
            return_exceptions=True
        )
        
        for engine, result in zip(engines, results):
            if isinstance(result, Exception):
                statuses[engine] = ApiStatus.unavailable
            else:
